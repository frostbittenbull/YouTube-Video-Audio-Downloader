...
(function () {
  ...
  function initYtdl(M, GV) {
    ...
    function gmRangeGet(url, start, end, onChunkProgress, controller) {
      return new Promise((resolve, reject) => {
        ...
        req = GM_xmlhttpRequest({
          ...
          anonymous: !!ua,
          headers: ua
            ? { 'Range': `bytes=${start}-${end}`, 'User-Agent': ua }
    ...
    const UA_BY_CLIENT = {};
    for (const c of INNERTUBE_CLIENTS) UA_BY_CLIENT[c.clientName] = c.userAgent;

    function uaForUrl(url) {
      try {
        return UA_BY_CLIENT[new URL(url).searchParams.get('c')] || null;
      } catch (e) {
        return null;
      }
    }
    ...
    async function downloadPairViaSabr(videoFmt, audioFmt, controller) {
    ...
    async function downloadAudioViaSabr(audioFmt, controller) {
      const cfg = buildSabrConfig();
      ...
      const audioBuf = await readStreamToBuffer(
        audioStream,
      ...
      try {
        ...
        if (videoDirect && audioDirect) {
          try {
            ...
            if (e && e.cancelled) throw e;
            console.warn('[ytdl] Прямые ссылки не сработали (' + e.message + '), переключаюсь на SABR');
            videoBuf = audioBuf = undefined;
            setStreamProgress('video', 'Видеопоток', 0, videoFmt.contentLength || 0);
            setStreamProgress('audio', 'Аудиопоток', 0, audioFmt.contentLength || 0);
          }
        }
        ...
        if (!directOk) {
          setModalStatus(videoDirect && audioDirect
            ? 'Прямые ссылки отклонены — переключаюсь на SABR…'
            : 'Прямой URL не найден — попытка через SABR…');
          ({ videoBuf, audioBuf, videoMime, audioMime } = await downloadPairViaSabr(videoFmt, audioFmt, controller));
        }
      ...
      try {
        ...
        if (direct) {
          try {
            ...
            if (e && e.cancelled) throw e;
            console.warn('[ytdl] Прямая ссылка не сработала (' + e.message + '), переключаюсь на SABR');
            audioBuf = undefined;
            setStreamProgress('audio', 'Аудиопоток', 0, audioFmt.contentLength || 0);
          }
        }
        ...
        if (!directOk) {
          setModalStatus(direct
            ? 'Прямая ссылка отклонена — переключаюсь на SABR…'
            : 'Прямой URL не найден — попытка через SABR…');
          ({ audioBuf, audioMime } = await downloadAudioViaSabr(audioFmt, controller));
        }
